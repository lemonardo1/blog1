...
서로 다른 샘플이나 다양한 데이터 유형을 **통합 분석**하는 단계입니다. 통합 분석은 크게 **멀티 샘플 통합**(여러 실험 배치의 scRNA-seq 데이터 통합)과 **멀티 모달 통합**(다른 종류의 single-cell 데이터 간 통합)으로 구분할 수 있습니다.
...
RNA velocity 분석은 단일 세포 RNA 시퀀싱 데이터를 이용해 각 세포의 미래 상태를 예측하는 기법입니다. 세포 내에서 전사체의 동적 변화를 파악하기 위해, 주로 다음과 같은 과정을 거칩니다:

- **전사체 스테이지 구분:** 미완성(unspliced) mRNA와 성숙(spliced) mRNA의 양을 비교합니다.
- **미래 상태 예측:** 현재 세포의 mRNA splicing 상태를 바탕으로, 앞으로 어떤 유전자 발현 패턴이 나타날지를 추정합니다.
- **시각화:** 보통 UMAP이나 t-SNE 같은 임베딩 차원에 각 세포의 진행 방향을 화살표(vector)로 표시하여, 세포들이 어떤 분화 경로를 밟고 있는지 시각적으로 표현합니다.

이 방법은 세포의 분화 과정이나 상태 전환을 이해하는 데 큰 도움을 주며, Velocyto와 같은 도구를 통해 수행됩니다.

### loom 파일이란?
loom 파일은 단일 세포 RNA 시퀀싱 데이터를 저장하기 위해 설계된 효율적인 파일 포맷입니다. 특히, Velocyto와 같은 RNA velocity 분석 도구에서 사용되는데요, 이 파일은 보통 HDF5 기반으로 만들어져 대규모의 행렬 데이터(예: spliced, unspliced transcript count 등)를 체계적으로 저장할 수 있습니다.

**주요 포인트:**

- **효율적인 데이터 저장:** loom 파일은 대용량의 행렬 데이터와 관련 메타데이터(세포 어노테이션 등)를 하나의 파일에 저장할 수 있어, 데이터 입출력이 빠릅니다.
- **RNA velocity 분석:** Velocyto는 세포 내 RNA splicing 상태(즉, spliced와 unspliced RNA)를 기반으로 RNA velocity를 계산합니다. 이를 위해 분석에 필요한 데이터를 loom 파일 형태로 저장하고 활용합니다.
- **유연성:** loom 파일은 단일 세포 데이터 분석에 최적화되어 있으며, 다양한 분석 도구와의 호환성을 고려하여 설계되었습니다.

요약하면, Velocyto에서 사용하는 loom 파일은 단일 세포 RNA 시퀀싱 데이터를 효율적으로 저장하고, RNA velocity 분석에 필요한 다양한 행렬 정보를 담는 파일 형식입니다.